	...
	public void run() {
		...
		try {
			...
			if(ServerInfo.getInstance().isCurrentServer(Config.getInstance().getBroadcastServer())){
				...
				DocumentCollection onlineUsers = db.getView(Const.VIEW_USERS_BY_STATUS).getAllDocumentsByKey(Const.STATUS_ONLINE,true);

				//exit if we're null or zero
				if(onlineUsers==null || onlineUsers.getCount() == 0) return;
